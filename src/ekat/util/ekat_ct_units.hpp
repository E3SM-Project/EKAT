#ifndef EKAT_COMPILE_TIME_UNITS_HPP
#define EKAT_COMPILE_TIME_UNITS_HPP
...
template<typename T>
struct ValidExp : std::is_integral<T> {};
template<int n, int d>
struct ValidExp<rational::RationalConstant<n,d>> : std::true_type {};

template<typename lengthExp, typename timeExp, typename massExp,
         typename temperatureExp, typename currentExp, typename amountExp,
         typename luminousIntensityExp>
class Units {
  static_assert(ValidExp<lengthExp>::value, "Invalid lengthExp type");
  static_assert(ValidExp<timeExp>::value, "Invalid timeExp type");
  static_assert(ValidExp<massExp>::value, "Invalid massExp type");
  static_assert(ValidExp<temperatureExp>::value, "Invalid temperatureExp type");
  static_assert(ValidExp<currentExp>::value, "Invalid currentExp type");
  static_assert(ValidExp<amountExp>::value, "Invalid amountExp type");
  static_assert(ValidExp<luminousIntensityExp>::value, "Invalid luminousIntensityExp type");
public:
  ...
  constexpr Units (const char* name)
  ...
  constexpr bool is_dimensionless () const {
    return compatible (nondimensional());
  }

  constexpr bool compatible (const Units&& u) const {
    return m_units[0]==u.m_units[0] &&
           m_units[1]==u.m_units[1] &&
           m_units[2]==u.m_units[2] &&
           m_units[3]==u.m_units[3] &&
           m_units[4]==u.m_units[4] &&
           m_units[5]==u.m_units[5] &&
           m_units[6]==u.m_units[6];
  }

  constexpr bool same_as (const Units&& u) const {
    return compatible(std::forward<const Units&&>(u)) && m_scaling==u.m_scaling;
  }
...
#endif // EKAT_COMPILE_TIME_UNITS_HPP
