...
#define EKAT_BINARY_MATH_EXPRESSION(impl,name) \
  ...
  public:                                                                                 \
    ...
    return_type eval (const TeamEvalSpecs<TeamMember>& specs, Args... args) const {       \
      if constexpr (not expr_l)                                                           \
        return eval_impl(m_arg1,m_arg2.eval(specs,args...));                              \
      else if constexpr (not expr_r)                                                      \
        return eval_impl(m_arg1.eval(specs,args...),m_arg2);                              \
      else                                                                                \
        return eval_impl(m_arg1.eval(specs,args...), m_arg2.eval(specs,args...));         \
    }                                                                                     \
  protected:                                                                              \
    KOKKOS_INLINE_FUNCTION                                                                \
...
#define EKAT_UNARY_MATH_EXPRESSION(impl,name) \
  ...
  public:                                                                           \
    ...
    KOKKOS_INLINE_FUNCTION                                                          \
    return_type eval (const TeamEvalSpecs<TeamMember>& specs, Args... args) const { \
      return Kokkos::impl(m_arg.eval(specs,args...));                               \
    }                                                                               \
  protected:                                                                        \
    EArg    m_arg;                                                                  \
  };                                                                                \
