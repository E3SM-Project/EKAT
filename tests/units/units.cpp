...
#include "ekat/util/ekat_ct_scaling_factor.hpp"
...
template<typename T>
struct Compiles<T,decltype(void(sizeof(T)))> : std::true_type {};

template<long long n, long long d>
using RC = ekat::rational::RationalConstant<n,d>;
template<long long nb, long long db, long long ne, long long de>
using SF = ekat::scaling::ScalingFactor<RC<nb,db>,RC<ne,de>>;

template<typename T, typename S>
constexpr bool same() { return std::is_same<T,S>::value; }

TEST_CASE ("ct_rc") {
  using namespace ekat::rational;
  std::cout << "RC<0,0> compiles: " << Compiles<RC<0,0>>::value << "\n";
  std::cout << "RC<0,1> compiles: " << Compiles<RC<0,1>>::value << "\n";
  std::cout << "pow(one,0) compiles: " << Compiles<pow_t<one_t,0>>::value << "\n";
  std::cout << "pow(zero,0) compiles: " << Compiles<PowImpl<zero_t,0,true>>::value << "\n";
  std::cout << "one==1/one: " << same<one_t,inv_t<one_t>>() << "\n";
  std::cout << "inv(zero) compiles: " << Compiles<inv_t<zero_t>>::value << "\n";
}

TEST_CASE ("ct_sf") {
  using namespace ekat;
  using namespace ekat::scaling;

  // using one_pow_zero = ScalingFactor<rational::one_t,rational::zero_t>;
  std::cout << "SF<0,0> compiles: "
    << Compiles<
                ScalingFactor<rational::zero_t,
                              rational::zero_t>
               >::value << "\n";

  using two = rational::RationalConstant<2>;
  using sqrt2 = ScalingFactor<two,rational::inv_t<two>>;

  std::cout << "sqrt2 base: " << sqrt2::base() << "\n";
  std::cout << "sqrt2 exp : " << sqrt2::exp() << "\n";
...
//   constexpr auto one = Units::nondimensional();
//   static_assert (one.is_dimensionless(),"ops");
